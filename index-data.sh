usage()
{
  echo "Usage: $0 [-y /path/to/python/indexing/code] [-o hostname where OpenSearch is running. Default is localhost] [-m max docs PER WORKER. Default: 2000000] [-w number of threads. Default: 8] [-d /path/to/kaggle/best/buy/datasets] [-p /path/to/bbuy/products/field/mappings] [ -g /path/to/write/logs/to ]"
...
WORKERS=8
MAX_DOCS=2000000
while getopts ':p:o:g:y:d:m:w:h' c
do
  case $c in
...
if [ -f $PRODUCTS_JSON_FILE ]; then
  ...
  if [ -f index.py ]; then
    ...
    nohup python index.py -w $WORKERS -m $MAX_DOCS -s "$DATASETS_DIR/product_data/products" -o "$HOST" > "$LOGS_DIR/index.log" &
    if [ $? -ne 0 ] ; then
