...
docker ps --filter "name=opensearch-node*" --filter "status=running" --format "{{.Names}}" | while read -r container_name; do
  ...
  docker exec -t $container_name ./bin/opensearch-plugin install "https://github.com/aiven/prometheus-exporter-plugin-for-opensearch/releases/download/2.11.0.0/prometheus-exporter-2.11.0.0.zip"
